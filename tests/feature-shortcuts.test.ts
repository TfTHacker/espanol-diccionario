...
import { VIEW_TYPE_DICTIONARY, VIEW_TYPE_SPANISH_CHAT, VIEW_TYPE_TTS_PRACTICE, VIEW_TYPE_TRANSLATOR } from "../src/constants";
...
test("feature shortcuts expose the same top-level custom forms in a stable order", () => {
  assert.deepEqual(
    ...
    [VIEW_TYPE_DICTIONARY, VIEW_TYPE_SPANISH_CHAT, VIEW_TYPE_TTS_PRACTICE, VIEW_TYPE_TRANSLATOR],
  );
});

test("feature shortcuts have compact icon labels and accessible titles", () => {
  assert.equal(FEATURE_SHORTCUTS.length, 4);
