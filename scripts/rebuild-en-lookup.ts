...
async function main() {
	...
	for (const s of samples) {
		const row = db3.prepare("SELECT id, word, pos FROM words WHERE word = ? AND lang = 'en'").get(s) as { id: number; word: string; pos: string } | undefined;
		if (row) {
			const defs = db3.prepare("SELECT definition FROM definitions WHERE word_id = ? ORDER BY sense_num ASC").all(row.id) as { definition: string }[];
